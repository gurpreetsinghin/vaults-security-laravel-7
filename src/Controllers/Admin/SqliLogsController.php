...
class SqliLogsController extends Controller
{

    use Config;

    public function index(){
        $logs = DB::table($this->prefix().'logs')->where('type', 'SQLi')->orderBy('id', 'desc')->get();
        return view('project-security::admin.sqli-logs.index', ['logs' => $logs]);
    }

    public function view($id){
        $log = DB::table($this->prefix().'logs')->find($id);
        return view('project-security::admin.sqli-logs.view', ['log' => $log]);
    }

    public function delete($id){
        try{
            DB::table($this->prefix().'logs')->where('id', $id)->delete();
            return redirect(route('ps.admin.sqli-logs'))->with('success', 'Log has been deleted successfully!');
    ...
    public function deleteAll(){
        try{
            DB::table($this->prefix().'logs')->where('type', 'SQLi')->delete();
            return redirect(route('ps.admin.sqli-logs'))->with('success', 'Sqli Logs has been deleted successfully!');
        }catch(\Illuminate\Database\QueryException $ex){
            return back()->with('error', $ex->getMessage());
        }
    }
