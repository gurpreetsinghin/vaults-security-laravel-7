        ...
        <div class="row">
            <div class="col-md-9">
                <div class="card card-dark">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('ps.admin.error-monitoring') }}">
                            @csrf
                            <div class="row">
                                <div class="col-md-6">
                                    <label><i class="fas fa-bug"></i> Error Reporting</label>
                                    <select class="form-control" name="error_reporting" style="width: 100%;">
                                        <option value="1" {{ $setting->error_reporting == 1 ? 'selected' : '' }}>Turned
                                            Off</option>
                                        <option value="2" {{ $setting->error_reporting == 2 ? 'selected' : '' }}>Report
                                            simple running errors</option>
                                        <option value="3" {{ $setting->error_reporting == 3 ? 'selected' : '' }}>Report
                                            simple running errors + notices</option>
                                        <option value="4" {{ $setting->error_reporting == 4 ? 'selected' : '' }}>Report
                                            all errors except notices</option>
                                        <option value="5" {{ $setting->error_reporting == 5 ? 'selected' : '' }}>Report
                                            all PHP errors</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label><i class="fas fa-eye"></i> Errors Visibility</label>
                                    <select class="form-control" name="display_errors" style="width: 100%;">
                                        <option value="0" {{ $setting->display_errors == 0 ? 'selected' : '' }}>Hide
                                            Errors</option>
                                        <option value="1" {{ $setting->display_errors == 1 ? 'selected' : '' }}>Display
                                            Errors</option>
                                    </select>
                                </div>
                            </div><br />

                            <input class="btn btn-primary btn-block btn-flat" type="submit" value="Save" />
                        </form>
                    </div>
                </div>
                <div class="card">
...
?>
            ...
            <div class="col-md-3">
                <div class="card card-dark">
                    ...
                    <div class="card-body">
                        Logging errors is recommended best practice, even for production site. Checking those logs
                        however might seem like a chore. The error monitoring brings all entries from error log on this
                        page.<br /><br />
                        <ul>
